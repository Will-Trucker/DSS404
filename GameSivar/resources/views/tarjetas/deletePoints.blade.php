...
<body>
    ...
    <div class="flex justify-center items-center h-screen w-full ">
        <div class="w-1/2 bg-white rounded shadow-2xl p-8 m-4">
            ...
            <form method="post" id="crearTarjetaForm" action="{{ route('tarjetas.update', $tarjetaPunto->id) }}" enctype="multipart/form-data">
              @csrf
                ...
                <div class="flex flex-col mb-4">
                    ...
                    <select name="tarjeta_juegos_id" id="tarjeta_juegos_id" class="border py-2 px-3 text-grey-800" value="{{old('tarjeta_juegos_id')}}">
                      ...
                      <option value="{{ $tarjeta->id }}" readonly selected>{{$tarjeta->tipo}} - {{ $tarjeta->codigo }}</option>
                  @endforeach
                    </select>
